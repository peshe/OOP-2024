...
Computer::Computer(const char *brand, const char *processor, unsigned video,
                   unsigned hdd, unsigned weight, unsigned batteryLife,
                   float price, unsigned quantity)
...
Computer::Computer(const Computer &other)
    : Computer(other.brand, other.processor, other.video, other.hdd,
               other.weight, other.batteryLife, other.price, other.quantity) {}

Computer::~Computer() noexcept { freeMemory(); }
...
/*static*/ Computer Computer::loadFromText(std::istream &istream) {
  ...
  if (!istream)
    throw "Problem reading from text file!";

  return Computer(brand, processor, video, hdd, weight, batteryLife, price,
                  quantity);
}
...
static const char *cstringFromBinary(std::istream &istream) {
  std::size_t len = 0;
...
/*static*/ Computer Computer::loadFromBinary(std::istream &istream) {
  ...
  const char *brand = cstringFromBinary(istream);
  const char *processor = cstringFromBinary(istream);
  ...
  Computer result(brand, processor, video, hdd, weight, batteryLife, price,
                  quantity);
  ...
  if (!istream)
    throw "Problem reading from binary file!";

  return result;
}

static void cstringAsBinary(const char *str, std::ostream &ostream) {
  assert(str != nullptr);
...
void Computer::saveAsBinary(std::ostream &ostream) const {
  ostream.write(reinterpret_cast<const char *>(&serialNumber),
                sizeof(serialNumber));
  cstringAsBinary(brand, ostream);
  cstringAsBinary(processor, ostream);
  ...
  ostream.write(reinterpret_cast<const char *>(&batteryLife),
                sizeof(batteryLife));
